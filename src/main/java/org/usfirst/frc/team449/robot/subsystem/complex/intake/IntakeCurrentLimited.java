...
import org.usfirst.frc.team449.robot.subsystem.interfaces.intake.SubsystemIntake;JsonIdentityInfo(generator = ObjectIdGenerators.StringIdGenerator.class)

public class IntakeCurrentLimited extends Subsystem implements SubsystemIntake, SubsystemConditional {
     ...
     */
    @JsonCreator
    public IntakeCurrentLimited(@JsonProperty(required = true) @NotNull FPSTalon motor,
                        @Nullable Double inSlowVel,
                        @Nullable Double inFastVel,
                        @Nullable Double outSlowVel,
                        @Nullable Double outFastVel,
                                @JsonProperty(required = true) double currentLimit) {
        this.motor = motor;
    ...
    @Override
    public void update() {
        this.condition = isConditionTrue()
    }
}