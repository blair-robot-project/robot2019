...
public class FeedForwardArmComponent extends FeedForwardKaKvComponent {
     ...
     * @param interceptVoltageFwd The voltage required to overcome static friction in the forwards direction with the arm horizontal. Vintercept in the drive characterization paper. Defaults to 0.
     * @param interceptVoltageRev The voltage required to overcome static friction in the reverse direction with the arm horizontal. Vintercept in the drive characterization paper. Defaults to interceptVoltageFwd.
    ...
    public double calcMPVoltage(double positionSetpoint, double velSetpoint, double accelSetpoint) {
        ...
        if (velSetpoint > 0 || (velSetpoint == 0 && accelSetpoint > 0)) {
            return velSetpoint * kVFwd + angleFactor * (accelSetpoint * kAFwd + interceptVoltageFwd);
        } else if (velSetpoint < 0 || (velSetpoint == 0 && accelSetpoint < 0)) {
            return velSetpoint * kVRev + angleFactor * (accelSetpoint * kARev - interceptVoltageRev);
        } else {
            return 0;
        }
    }
    ...
    public double applyAsDouble(double operand) {
        return super.applyAsDouble(operand) * Math.cos((talon.getPositionFeet() + angleOffset)*2*Math.PI);
    }
}
